...
import platform
import re
from utils import generateCLICommands
...
if sys.version_info < (3, 7, 0):
	sys.exit(
	    "Script requires at least python 3.7, please upgrade and try again."
	)
...
def get_config() -> tuple[str, str, str]:
	...
	host = config.get(
	    "Client", "host", fallback=default_config["Client"]["host"]
	)
	username = config.get(
	    "Client", "username", fallback=default_config["Client"]["username"]
	)
	password = config.get(
	    "Client", "password", fallback=default_config["Client"]["password"]
	)
...
def make_new_config(
	...
	host = input(
	    f"Enter qBittorrent Web UI host (Empty to use {default_config['Client']['host']}): "
	)
	host = host.strip() or default_config["Client"]["host"]
	print(f"Using qBittorrent Web UI host: {host}")

	username = input(
	    f"Enter qBittorrent Web UI username (Empty to use {default_config['Client']['username']}): "
	)
	username = username.strip() or default_config["Client"]["username"]
	print(f"Using qBittorrent Web UI username: {username}")

	password = input(
	    f"Enter qBittorrent Web UI password (Empty to use {default_config['Client']['password']}): "
	)
	password = password.strip() or default_config["Client"]["password"]
	print(
	    "Using qBittorrent Web UI password: *****"
	)  # For security, we only print asterisks for the password
...
def are_all_paths_same(
	...
	for path in paths:
		try:
			...
			file_identifier: tuple[int,
			                       int] = (file_stat.st_dev, file_stat.st_ino)
...
def hardlink_largest_file(
	...
	existing_files: set[Path] = {
	    Path(file).resolve()
	    for file in matching_files if Path(file).resolve().exists()
	}
	...
	for file in matching_files:
		...
		if r_file.exists() and r_file.is_dir(
		):  # unlink will fail if it's somehow a folder.
...
def get_matching_files_in_dir_and_subdirs(
	...
	files_in_directory: list[str] = [
	    ...
	    for dirpath, _, filenames in os.walk(search_path) for name in filenames
	]
	...
	if ignore_sizes:
		return [pair for pair in files_and_sizes]
	else:
...
def match(
    ...
    allow_download_existing: bool,
    invert_select: bool,
) -> bool:
	file_list_to_delete: list = []
	...
	for torrent_file in torrent.files:  # type: ignore[reportAttributeAccessError]
		if torrent_file.priority == 0:
			if allow_download_existing or invert_select:
				pass
			else:
				continue
		...
		original_file_path_str: str = str(original_file_path)
		original_file_path_pattern = rf"^{re.escape(original_file_path_str)}$"
		if allow_duplicated:
			matching_files: list[str] = [
			    disk_file_abs_path
			    for disk_file_abs_path, disk_file_size in files_in_directory
			    if disk_file_abs_path == original_file_path_str
			    # if re.match(original_file_path_pattern,disk_file_abs_path)
			]
		else:
			matching_files: list[str] = [
			    disk_file_abs_path
			    ...
			    if torrent_file.size == disk_file_size and (
			        not match_extension or Path(disk_file_abs_path).suffix.
			        lower() == original_relpath.suffix.lower()
		...
		if len(matching_files) > 1:
			...
			if is_dry_run:
				print(
				    "Multiple files found (dryrun - would normally prompt to select)"
				)
				continue
			...
			question: list[dict[str, Any]] = [
			    {
			        "type":
			            "list",
			        "message":
			            f"Multiple matches found (all {torrent_file.size} bytes) for '{original_relpath_str}'. Select a file to match:",
			        "choices":
			            choices,
			        "name":
			            "file",
			...
			if response["file"] == hardlink_option:
				...
				if torrent_file.priority in (0, "0"):
					print(
					    f"setting file priority of '{torrent_file.name}' to 1."
					)
			...
			), f"Invalid response: '{selected_file_path}'"
			...
			if invert_select:
				if torrent_file.priority == 0:
					file_list_to_delete.append(str(selected_file_path))
					continue
				else:
					continue
			...
			print(
			    f"{Fore.YELLOW}No matches found for '{original_relpath_str}'!{Style.RESET_ALL}"
			)
			if no_redownload:
		...
		new_relative_path: Path = Path(selected_file_path
		                              ).relative_to(download_path)
		new_relative_path_str: str = new_relative_path.as_posix()
		if new_relative_path == original_relpath:
			if torrent_file.priority == 0:
				if allow_download_existing and not is_dry_run:
					torrent.file_priority(
					    file_ids=int(torrent_file.index),
					    priority=1,
					)
					torrent_file.priority = 1
			if torrent_file.priority >= 1:
				print(f"{original_relpath_str} already synced, left as is")
			else:
				print(
				    f"{Fore.YELLOW}{original_relpath_str} already synced, but not set to download{Style.RESET_ALL}"
				)
			continue
...
def set_search_and_download_paths(
	...
	if input_download_path:
		content_path = input_download_path.joinpath(
		    content_path.relative_to(raw_download_path)
		)
	...
	if input_search_path:
		if not is_relative_to(input_search_path, download_path):
			print(
			    f"Skipping {torrent.name}: Search path {input_search_path} must be a sub directory of {raw_download_path}\n"
			)
			return None, None
		...
		search_path = content_path if content_path.is_dir(
...
def matcher(
    ...
    allow_download_existing: bool = False,
    invert_select: bool = False,
    ignore_sizes: bool = False,
):
	qb_client: Client = init_client(
	)  # this doesn't mean we actually connected yet.
	if input_torrent_hashes:
		torrents: TorrentInfoList = qb_client.torrents.info(
		    torrent_hashes=input_torrent_hashes
		)
		print("Connected to api!")
		if not torrents:
			sys.exit(
			    f"{Fore.RED}No torrents found matching any of the provided hashes.{Style.RESET_ALL}"
			)
		else:
			found_hashes: list[str] = [
			    torrent["hash"].upper() for torrent in torrents
			]  # Extracting found hashes
			for hash_value in input_torrent_hashes:
				if hash_value not in found_hashes:
					print(
					    f"{Fore.RED}Torrent with hash '{hash_value}' not found.{Style.RESET_ALL}"
					)
	elif sync_all:
		...
		if not torrents:
			sys.exit(
			    f"{Fore.RED}No torrents found found anywhere in your qBittorrent{Style.RESET_ALL}"
			)
	else:
	...
	for torrent in torrents:
		...
		print(
		    f"Search directory '{search_path}'\nDownload directory '{download_path}'"
		)

		# Unfortunately hashing individual files isn't possible (or at least practical), so we match with their sizes.
		torrent_file_sizes: set[int] = {
		    file.size
		    for file in torrent.files if file.size
		}

		print("Scanning files in search directory")
		files_in_directory: list[tuple[str, int]
		                        ] = get_matching_files_in_dir_and_subdirs(
		                            search_path, torrent_file_sizes,
		                            use_hardlinks, ignore_sizes
		                        )
		print(f"Found {len(files_in_directory)} matches in '{search_path}'")
		made_change: bool = match(
		    torrent, files_in_directory, match_extension, download_path,
		    use_hardlinks, is_dry_run, no_redownload, allow_duplicated,
		    allow_download_existing, invert_select
		)
		if invert_select:
			for file in made_change:
				if platform.system() == "Windows":
					commandOutput = generateCLICommands(
					    operation="delete", target=file, forceConfirm=True
					)
				else:
					commandOutput = generateCLICommands(
					    operation="trash", target=file, forceConfirm=True
					)
				print(commandOutput)
		elif input_download_path and input_download_path != torrent.save_path and not is_dry_run:
			print(f"Changing torrent save location to {input_download_path}")
			qb_client.torrents_set_location(
			    torrent_hashes=torrent_hash, location=str(input_download_path)
			)
		...
		if is_dry_run:
			print(
			    f"{Fore.YELLOW}Performed a dry run, nothing was modified{Style.RESET_ALL}"
			)
...
def main() -> None:
	...
	parser = argparse.ArgumentParser(
	    description=
	    "Tool to match torrents added to qBittorent to files on a disk"
	)
	parser.add_argument(
	    "input",
	    nargs="?",
	    default=None,
	    help="Torrent hash, or a txt with a list of hashes."
	)
	parser.add_argument(
	    "-a",
	    "-all",
	    action="store_true",
	    help=
	    "Look for matches for every qBT torrent. Ignored if used with input hash(es)."
	)
	parser.add_argument(
	    "-s",
	    "-spath",
	    default=None,
	    help="Specifies search path. Must be a subpath of the download path."
	)
	parser.add_argument(
	    "-d",
	    "-dpath",
	    default=None,
	    help="Sets new download path for the torrent."
	)
	parser.add_argument(
	    "-fd",
	    action="store_true",
	    help=
	    "Forces search in torrent's download directory. Default is torrent's content directory. Ignored if passed along with search."
	)
	parser.add_argument(
	    "-e",
	    "-ext",
	    action="store_true",
	    help="Forces matched files to share an extension."
	)
	parser.add_argument(
	    "-dry",
	    action="store_true",
	    help="Performs a dry run without modifying anything."
	)
	parser.add_argument(
	    "-l",
	    "-link",
	    action="store_true",
	    help="Creates hardlinks instead of renaming."
	)
	parser.add_argument(
	    "-nodl",
	    "-no_download",
	    action="store_true",
	    help=
	    "If file not found on disk, tell qBittorrent to set priority of that file to 0."
	)
	parser.add_argument(
	    "-ad", "-allow_duplicated", action="store_true", help="test"
	)
	parser.add_argument(
	    "-ade",
	    "-allow_download_existing",
	    action="store_true",
	    help="Allow enable files's download priority."
	)
	parser.add_argument(
	    "-inv",
	    "-invert_select",
	    action="store_true",
	    help="list files to delete based on files's priority."
	)
	parser.add_argument(
	    "-ig",
	    "-ignore_sizes",
	    action="store_true",
	    help=
	    "ignore files's sizes while matching them. Should be enabled if files have not downloaded."
	)
	...
	):  # TODO: determine whether it's a hash or a filepath.
	...
	if input_search_path and (
	    not input_search_path.exists() or input_search_path.is_file()
	):
		sys.exit(
		    f"bad search path: '{input_search_path}' (either nonexistent or not a directory)"
		)

	input_download_path: Path | None = Path(args.d) if args.d else None
	if input_download_path and (
	    not input_download_path.exists() or input_download_path.is_file()
	):
		sys.exit(
		    f"bad download path: '{input_download_path}' (either nonexistent or not a directory)"
		)

	if args.a and args.input:
		parser.print_help()
		sys.exit("Cannot use both '-a' and input hash in the same command.")
	if args.ade and args.inv:
		parser.print_help()
		sys.exit("Cannot use both '-ade' and '-inv' in the same command.")
	if not args.input and not args.a:
		parser.print_help()
		sys.exit(
		    "Nothing to do? (must pass `-all` OR an input torrent hash/file)"
		)
	...
	matcher(
	    ...
	    is_dry_run=args.dry,
	    allow_duplicated=args.ad,
	    allow_download_existing=args.ade,
	    invert_select=args.inv,
	    ignore_sizes=args.ig,
	)
