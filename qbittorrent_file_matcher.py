    ...
    for torrent_file in torrent.files:  # type: ignore[reportAttributeAccessError]
        ...
        if new_relative_path == original_relpath:
            ...
            if torrent_file.priority >= 1:
                ...
                print(f"{Fore.YELLOW}{original_relpath_str} already synced, but not set to download{Style.RESET_ALL}")
            continue
...
def matcher(
    ...
    for torrent in torrents:
        ...
        files_in_directory: list[tuple[str, int]] = get_matching_files_in_dir_and_subdirs(search_path, torrent_file_sizes, use_hardlinks,ignore_sizes)
        print(f"Found {len(files_in_directory)} matches in '{search_path}'")
        made_change: bool = match(torrent, files_in_directory, match_extension, download_path, use_hardlinks, is_dry_run, no_redownload,allow_duplicated,allow_download_existing,invert_select)
        if invert_select:
            for file in made_change:
                if platform.system()== "Windows":
                    commandOutput=generateCLICommands(operation="delete",target=file,forceConfirm=True)
                else:
                    commandOutput=generateCLICommands(operation="trash",target=file,forceConfirm=True)
                print(commandOutput)
...
def main() -> None:
    ...
    parser.add_argument("-ad", "-allow_duplicated", action="store_true", help="test")
    parser.add_argument("-ade", "-allow_download_existing", action="store_true", help="Allow enable files's download priority.")
    parser.add_argument("-inv", "-invert_select", action="store_true", help="list files to delete based on files's priority.")
    parser.add_argument("-ig", "-ignore_sizes", action="store_true", help="ignore files's sizes while matching them. Should be enabled if files have not downloaded.")
